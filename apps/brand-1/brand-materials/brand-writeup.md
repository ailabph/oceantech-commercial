# Oceantech Offshore Diving Services — Brand Writeup

> Website copy direction document
> Brand 1 — Seasoned, dependable, warm, precise, grounded
>
> **Location:** Maribago, Lapu-Lapu City, Cebu, Philippines
> **Brand Colors:** Deep Ocean Teal `#0A4D68` | Sunset Orange `#E85D04` | Copper/Bronze `#B87333`
> **Tone:** Confident but not boastful. Direct. Warm. Technically precise when needed. Proud of Cebu roots.

---

## 1. Taglines

1. **Decades below. Built to deliver.**
2. **Where experience meets the deep.**
3. **Cebu-rooted. Ocean-proven.**
4. **Forty-five years in the water. Every job done right.**
5. **Deep work, done well.**

---

## 2. Hero Section Copy

**Headline (Primary):**
Commercial diving built on 45 years of ocean experience.

**Subheadline:**
From Cebu's waters to yours — underwater welding, inspection, hull maintenance, and structural repair. Trusted by the Philippines' top shipping lines since 2016.

**CTA Button Text:**
- Option A: Request a Project Quote
- Option B: Talk to Our Team

**Supporting Stat Callouts:**

| Stat | Label |
|------|-------|
| 45+ | Years in the water |
| 24/7 | Emergency response ready |
| Cebu-based | Rapid mobilization across the Visayas and beyond |

---

## 2a. Trusted By Section Copy

**Overline:**
Trusted by the Philippines' top shipping lines

**Body:**
From the country's largest shipping conglomerate to Cebu's port authority — our client list speaks for itself.

---

## 3. About Section Copy

**Section Headline:**
New company. Old hands.

**Paragraph 1:**
Oceantech Offshore Diving Services was established in 2016, but our story starts much earlier. Our founders have been in the water since 1981 — first as recreational divers working Cebu's reefs, then as underwater professionals serving the maritime and construction industries across the Philippines. That's over 45 years of reading currents, solving problems at depth, and bringing people home safe. In fact, the family behind Oceantech — the operators of a PADI dive shop at Costabella Resort — were pioneer PADI instructors in the Philippines, a distinction that speaks to the depth of our roots in this industry.

**Paragraph 2:**
When we formed Oceantech, the goal was straightforward: take everything we'd learned in those years and build a commercial diving company that does things the right way. No shortcuts on safety. No guesswork on execution. Just experienced divers, proper equipment, and honest work — the kind of outfit we'd want to hire if we were on the other side of the table.

**Paragraph 3:**
Based in Maribago, Lapu-Lapu City, we're proud to operate from the heart of the Central Visayas. Cebu's maritime heritage runs deep, and so does ours. This is a family business with deep roots in Cebu's diving community — and that personal connection to the water and the people who work on it is something no corporate outfit can replicate. We know these waters, we know this industry, and we show up ready to work.

**Paragraph 4:**
We're proud to count some of the Philippines' most recognized shipping lines among our clients — including 2GO Group, OceanJet, Trans-Asia, Lite Ferries, Cokaliong, SuperCat, FastCat, Starlite, Weesam Express, and Philippine Span Asia. We also serve the Cebu Port Authority, OPASCOR, and Holcim Philippines. When the companies that move the country trust you with their vessels and infrastructure, that says more than any brochure.

**Heritage Callout Line:**
_Recreational diving since 1981. Commercial diving since 2016. The experience of a lifetime, working for you._

---

## 4. Services Section Copy

**Section Headline:**
What we do below the surface.

**Intro Paragraph:**
We handle the full range of commercial diving work — from routine hull cleaning to complex underwater welding and structural repair. Every job gets the same level of preparation, the same safety standards, and the same crew discipline. Here's what we bring to the table.

---

### Underwater Welding
**Reliable welds in demanding conditions.**
Our certified divers perform wet and habitat welding on hulls, pipelines, and submerged structures. We work to code, in current, and on your schedule — because downtime costs money.

### Hull Cleaning
**Keeping your vessel efficient and seaworthy.**
Marine growth slows you down and drives up fuel costs. We provide thorough hull cleaning services that restore hydrodynamic performance and extend the life of your coatings, with minimal disruption to your operations.

### Pier & Wharf Cleanup
**Maintaining the structures your operations depend on.**
Piers, wharves, and port infrastructure take a beating from the sea. We handle debris removal, marine growth clearing, and underwater structural assessments to keep your waterfront facilities in working order.

### Ship & Vessel Maintenance
**Keeping fleets operational without dry dock delays.**
Not every repair needs a dry dock. Our divers perform propeller polishing, anode replacement, valve servicing, and general underwater maintenance — so your vessels stay in service longer between scheduled haul-outs.

### Underwater Cutting
**Precision cutting where access is limited to divers.**
We perform oxy-arc and thermic cutting on submerged steel, concrete, and mixed structures. Whether it's salvage work, decommissioning, or clearing obstructions, our crew handles it safely and efficiently.

### Survey & NDT Inspection
**Clear reporting on what's happening below the waterline.**
We conduct underwater surveys and non-destructive testing — including ultrasonic thickness measurements, visual inspections, and CCTV documentation — to give you an accurate picture of structural condition. Our reports are detailed, honest, and built for decision-making.

---

## 5. Process Section Copy

**Section Headline:**
How we work.

### Step 1 — Project Brief
You tell us what you need. We listen, ask the right questions, and scope the job. Whether it's a quick hull inspection or a multi-phase repair, every project starts with a clear understanding of your requirements, timeline, and constraints.

### Step 2 — Site Assessment
Before anyone goes in the water, we evaluate the site. Water conditions, structural context, access points, safety considerations — we plan the dive and dive the plan. No surprises.

### Step 3 — Execution
Our crew mobilizes with the right equipment and the right people for the job. Work is carried out to specification, with constant communication between the dive team and surface support. Safety protocols are followed on every dive, every time.

### Step 4 — Reporting
When the work is done, you get a complete report — photos, video, measurements, and findings. We document everything so you have a clear record for your files, your class society, or your next maintenance decision.

---

## 6. Why Us Section Copy

**Section Headline:**
Six reasons to work with Oceantech.

### 1. Safety Is Not Negotiable
Every dive follows a safety management plan. Every crew member is trained to international standards. We've built our reputation on bringing people home — and we intend to keep it that way.

### 2. Certified and Qualified
Our divers hold recognized commercial diving certifications, and our welders are coded to industry standards. We carry the paperwork because we've done the work to earn it. Our founders were pioneer PADI instructors in the Philippines — a pioneering distinction that set the standard for professional diving in the country and remains a cornerstone of our credibility today.

### 3. Rapid Mobilization from Cebu
Based in Lapu-Lapu City, we're positioned to respond fast across the Visayas and the wider Philippines. Local presence means shorter lead times, lower mobilization costs, and a team that knows the local waters and conditions.

### 4. The Right Tools for the Job
We invest in properly maintained diving systems, cutting and welding equipment, inspection technology, and surface-supplied diving setups. Good equipment makes good work possible — we don't cut corners on kit.

### 5. Experienced Crew, Not Just Certified Divers
Certifications get you in the water. Experience gets the job done. Our team has been diving commercially and recreationally for decades. They've seen the conditions, handled the problems, and delivered the results.

### 6. Transparent Pricing
We quote what the job costs. No hidden fees, no surprise line items after the fact. You'll know what you're paying for before we pick up a regulator.

---

## 7. CTA / Contact Section Copy

**Headline:**
Let's talk about your next project.

**Supporting Paragraph:**
Whether you need a quote, want to discuss a scope of work, or just have a question about what's possible underwater — reach out. We're straightforward people, and we're happy to talk through what you need.

**Button/Link Text Options:**
- Request a Quote
- Get in Touch
- Call Our Team

---

## 8. Footer Tagline

_Oceantech Offshore Diving Services — deep work, done well._
